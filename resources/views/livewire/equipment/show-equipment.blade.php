<div class="bg-[#101010] min-h-screen text-white flex flex-col p-8 gap-y-5">
            ...
            <div class="p-5 gap-2 flex flex-col items-center bg-[#111827] ">
                <form wire:submit='create' class="flex flex-col gap-y-3  w-full">
                    ...
                    <label class="w-full">
                        ...
                        <input class="w-full border border-gray-600 rounded-lg p-2 bg-gray-800 text-white focus:ring-2 focus:border-none focus:ring-[#fee814] focus:outline-none" type="file" wire:model="image">
                        @error('image')
                            <em class="text-red-500">{{ $message }}</em>
                        @enderror
                        @if ($image)
    ...
    <div class="flex items-center gap-x-16">
        <input class="w-1/2 bg-[#1f2937] focus:ring-2 focus:border-none focus:ring-[#fee814] focus:outline-none rounded-md" type="text" wire:model.live="search" placeholder="search">
        <select class="w-1/4 bg-[#1f2937] focus:ring-2 focus:border-none focus:ring-[#fee814] focus:outline-none rounded-md" wire:model.live="equipmentType">
        ...
        <button class="px-4 py-2 w-1/6 bg-yellow-500 text-white rounded-md  transition" wire:click="resetSearch">Reset Filter</button>
    </div>
            ...
            <div class="flex flex-col w-[30vw] min-h-[40vh] bg-[#1f2937] rounded-md">
                ...
                <div class="flex items-center justify-between p-4">
                    <div class="flex flex-col">
                        <h1 class="">Reference : <span class="text-yellow-500">{{ $equipment->reference }}</span></h1>
                        <h1 class="">Mark : <span class="text-yellow-500">{{ $equipment->mark }}</span></h1>
                        <h1 class="">Equipment Type : <span class="text-yellow-500">{{ $equipment->equipment_type }}</span></h1>
                    ...
                    <div class="flex flex-col items-center gap-y-3">
                        ...
                        <x-modal name="delete-equipment{{ $equipment->id }}" :show="$errors->isNotEmpty()">
                            <div class="p-5 gap-2 flex flex-col items-center bg-[#111827]">
                                ...
                                <div>
                                    <button x-on:click="$dispatch('close')" class="px-4 py-2 bg-gray-600 text-white rounded-lg  transition-colors duration-300">
                                        {{ __('Cancel') }}
                                    </button>
                                    <button type="button"
                        ...
                        <x-modal name="edit{{ $equipment->id }}" :show="$errors->isNotEmpty()">
                            <div class="p-5 gap-2 flex flex-col items-center bg-[#111827]">
                                <form wire:submit='update({{ $equipment->id }})' class="flex flex-col w-full gap-y-3">
                                    ...
                                    <label class="flex flex-col gap-y-2">
                                        <span>mark :</span>
                                        <input type="text" wire:model="mark" class="w-full border border-gray-600 rounded-lg p-2 bg-gray-800 text-white focus:ring-2 focus:border-none focus:ring-[#fee814] focus:outline-none">
                                        @error('mark')
                                            <em class="text-red-500">{{ $message }}</em>
                                        @enderror
                                    </label>
                                    ...
                                    <label class="flex flex-col gap-y-2">
                                        ...
                                        <select wire:model="equipment_type" class="w-full border border-gray-600 rounded-lg p-2 bg-gray-800 text-white focus:ring-2 focus:border-none focus:ring-[#fee814] focus:outline-none">
                                            <option disabled>choose equipment type</option>
                                            <option value="camera">camera</option>
                                            <option value="son">son</option>
                                            <option value="lumiere">lumiere</option>
                                    ...
                                    <label class="w-full">
                                        <input type="file" wire:model="image" class="w-full border border-gray-600 rounded-lg p-2 bg-gray-800 text-white focus:ring-2 focus:border-none focus:ring-[#fee814] focus:outline-none">
                                        @error('image')
                                            <em class="text-red-500">{{ $message }}</em>
                                        @enderror
                                    </label>
                                    <div class="flex items-center justify-end gap-x-3">
                                        <button wire:click="cancel" type="button" x-on:click="$dispatch('close')" class="px-4 py-2 bg-gray-600 text-white rounded-lg  transition-colors duration-300">
                                            {{ __('Cancel') }}
                                        </button>
                                        <button type="submit"
